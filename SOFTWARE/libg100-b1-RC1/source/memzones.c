...
/*********************************************
**  memory_zone_libraries release 1.06	    **
**					    **
**  Copyright (c) John Wellesz 2001 - 2004  **
**  All rights Reserved.		    **
*********************************************/
...
/*
 * Read/write/create Routines for memory zone. version 1.06
...
#define _DS (__segment)(&_BALIB_segm_video)
...
long search_mem_zone(int zone_id, unsigned char *zone_name, struct memory_zone *bf)
{
	...
	if (b == 8 || zone_name[b] == curent_b_ptr[3 + b]) {
	    found = 1;
	    break;
	} else b=0;//Corrects the bug found by David Quaranta (b was never reset to 0).
...
{
    if (length + offset > bf->b_real_size) {
	if (offset >= bf->b_real_size) return 0;
...
unsigned int write_mem_zone(struct memory_zone *bf, const void *src, unsigned offset, unsigned length)
{
    ...
    //      --> length = 1
    //   --> length 0  offset 10
    //      -->  return 0 (my_movedata will do nothing)
    if (length + offset > bf->b_real_size) {
	if (offset >= bf->b_real_size) return 0;
	length = bf->b_real_size - offset;
    }
...
// void init_area {{{
void init_area(u_int b_segment, u_int b_offset, u_int msize, u_int value, int direction)
{
    u_long transit;
    if (!msize) return;
