...
{ int i,j;
  ...
  j=i=0;
  d8_and(i,j,sprite[2],SEG1); // on pose le masque du sprite sur les 2 buffers
  d8_and(i,j,sprite[2],SEG3);
  d8_or(i,j,sprite[0],SEG1);  // puis on affiche le sprite sur les 2 buffers
  d8_or(i,j,sprite[1],SEG3);
  do{ temps(150);  // on ralentit un peu le programme
